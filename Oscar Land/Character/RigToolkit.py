...
class WrestlerToolkit(object) :
    ...
    def updateCheckboxUI(self):
        moveAll = self.getAnim('fk_c_moveAll_anim')
        for attr in self.checkboxDict :
            onOff = pm.getAttr(moveAll + '.' + attr)
            pm.checkBox(self.checkboxDict[attr], edit = True, value = onOff)

    def setVisibilityToUI(self, args):
        moveAll = self.getAnim('fk_c_moveAll_anim')
        for attr in self.checkboxDict :
            pm.setAttr(moveAll + '.' + attr, pm.checkBox(self.checkboxDict[attr], query = True, value = True))
    ...
    def getAnim(self, animName):
        animList = pm.ls(animName, r = True)
        anim = None
        if len(animList) > 0 :
            anim = animList[0]
        return anim
